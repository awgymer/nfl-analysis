...
behind_q4 <- ggplot(melted_qb_summ[ahead == F], aes(x=plays, y=val)) + 
  ...
  facet_wrap(~stat, scales='free_y', 
             ...
             strip.position = 'left') +
  labs(
    title = 'QB EPA and WPA Chasing the Game in Q4',
...
ahead_q4 <- ggplot(melted_qb_summ[ahead == T], aes(x=plays, y=val)) + 
  ...
  facet_wrap(~stat, scales='free_y', 
             ...
             strip.position = 'left') +
  labs(
    title = 'QB EPA and WPA Closing Out the Game in Q4',
    subtitle = 'Score Differential > 0 / Min 75 Dropbacks',
    x = 'Dropbacks', y = NULL
  ) +
  theme_nfl() + 
...
bplot_behind <- ggplot(
  ...
  labs(
    title = "WPA of Passes When Chasing the Game in Q4",
...
bplot_ahead <- ggplot(
  ...
  geom_hline(aes(yintercept = mean(wpa, na.rm = T)), linetype = 2, colour = 'red') +
  labs(
    title = "WPA of Passes When Closing Out the Game in Q4",
    subtitle = "Min 75 Att / Red line is mean WPA of all featured QBs / QBs ordered by mean WPA low to high", 
    y = 'WPA', x = NULL
  ) +
  theme_nfl() +
